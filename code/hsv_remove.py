...
def load_and_convert_to_hsv(image_path):
    ...
    hsv_image = cv2.cvtColor(rmbg_image, cv2.COLOR_BGR2HSV)
    return hsv_image
...
def plot_image(image_path, ax, title):
    image = remove(cv2.imread(image_path))
    ...
    # Add vertical lines to divide the image into three equal columns
    for i in range(1, 3):
        ax.axvline(x=width * i / 3, color='gray', linestyle='-', linewidth=0.5)
    
    # Add horizontal lines to divide the image into three equal rows
    for i in range(1, 3):
        ax.axhline(y=height * i / 3, color='gray', linestyle='-', linewidth=0.5)
...
def main():
    # Paths to the images
    image_path1 = "image008.png"
    image_path2 = "image009.png"
