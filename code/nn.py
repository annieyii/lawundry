...
def read_and_preprocess():
    ...
    df = pd.read_csv('code/TradeMark.csv')
    ...
    lambda row: calculate_ssim('code/imagesdata/' + row['firstColumn'], 'code/imagesdata/' + row['secondColumn']),
    axis=1
    )
    ...
    new_df['hsvScore'] = new_df.apply(
        lambda row: calculate_hsv_similarity('code/imagesdata/' + row['firstColumn'], 'code/imagesdata/' + row['secondColumn']),
        axis=1
    )
